...
# - Fast port conflict detection (single cached ss/netstat snapshot)
# - IPv6 firewall awareness (UFW)
# - Logging (/var/log/sshport_manager.log) (key events only)
# - Fast IP detection (1s timeout)
#
# NOTE: Run as root.
...
# ---------- cached snapshot for port checks (run once) ----------
...
# returns 0 if free or bound by sshd, 1 if conflict
check_port_conflict() {
    local port=$1
    local snapshot
...
# Set SSH port (remove all Port lines then add one)
set_ssh_port() {
    local port=$1
    backup_sshd_config
